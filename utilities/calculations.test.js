...
describe("calculations", () => {
  ...
  // activitiesByModule is the output of calculations.filterModuleActivities()
  ...
  it('Should be able to return all activities filtered by module', () => {
    expect(calculations.filterModuleActivities(courseActivityTotals, 74)).toStrictEqual([
      {
        activityId: 1,
  ...
  it.only('Should be able to return all the activities and percentage of totals within a module', () => {
    console.log('3', calculations.getModulePercentages(activitesByModule))
  ...
  it("should return all the activities for the course", () => {
    expect(calculations.getPercentages(allActivityTotals)).toStrictEqual(
      data.coursePercentageData
    );
  });

  it("should return the activity data with a name, percentage and module id", () => {
    expect(calculations.getPercentages(allActivityTotals)[0]).toStrictEqual(
      data.coursePercentageData[0]
    );
    expect(calculations.getPercentages(allActivityTotals)[0].name).toStrictEqual(
      "Reading (understand)"
    );
    expect(
      calculations.getPercentages(allActivityTotals)[0].percentage
    ).toStrictEqual(7);
    expect(
      calculations.getPercentages(allActivityTotals)[0].moduleId
    ).toStrictEqual(1);
  });

  it("should return the total percents of all the activities in the course", () => {
    expect(
      calculations.getPercentages(allActivityTotals, "activity")
